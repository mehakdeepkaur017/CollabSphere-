...
import { useQuickFiles } from "@/features/workspace/useFileQueries"

export function QuickFilesCard() {
  const { activeWorkspace } = useWorkspaceStore()
  const { data: files, isLoading } = useQuickFiles(activeWorkspace?._id)
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold text-white">Quick Files</h2>
        </div>
      ...
      <div className="flex-1 flex flex-col min-h-[240px]">
        {isLoading ? (
          ...
          <div className="flex flex-col gap-3">
            {files.map(file => (
              <div key={file.id} className="flex items-center justify-between rounded-xl border border-white/5 bg-white/5 p-3 hover:bg-white/10 cursor-pointer transition-colors">
                <div className="flex items-center gap-3">
                  <Icons.fileText className="h-4 w-4 text-blue-400/70" />
                  <span className="font-medium text-white/90 text-sm">{file.name}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-1 flex-col items-center justify-center text-center">
            ...
            <p className="text-sm text-white/50 mb-4">No recent uploads.</p>
